...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    /**
     * Get authenticated user identity from Cloudflare Access headers
     * GET /api/me
     */
    app.get('/api/me', (c) => {
        ...
        if (!email) {
            return c.json({ success: false, user: null, error: 'Unauthenticated' }, 401);
        }

        return c.json({
            success: true,
            user: { email, jwt: jwt ? 'present' : 'absent' }
        });
    });
